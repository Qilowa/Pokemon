...
public class PokemonTeam implements Serializable{
	...
	public int createTeam(Pokedex p) {
		...
		System.out.println("Voulez-vous reprendre votre partie ou en recommencer une nouvelle ? (0: Créer, 1: Chargé) :");
	    int game = sc.nextInt();
	    while(game!=0 && game!=1) {
	    ...
	    if (game==1) {
			System.out.println("Team Chargé");
			return 1;
		}
	    else {
	...
	public boolean haveLost() {
		if (this.isEmpty()) {
			throw new IllegalStateException("S’il vous plaît ajouter un Pokémon à votre équipe");
		}
